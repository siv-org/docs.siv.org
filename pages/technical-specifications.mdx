...
Three essential pre-election steps, labeled as a, b, and c, must be completed in any order before the election can begin.

### a. Compile Voter Roll

The Election Administrator is responsible for defining a list of eligible voters. Each voter is assigned a unique Voter Authorization Token ("Auth Token").
...
These Auth Tokens are currently 10-character hexadecimal strings (e.g., 2378bf376d), resulting in $16^{10}$ (= $2^{40}$) or slightly over a trillion possible combinations. Brute force attempts to crack Auth Tokens are not a significant risk, as IP-address-based validation logging and rate-limiting are in place.

Election Administrators have knowledge of all assigned Voter Auth Tokens. When the election begins, each Voter Auth Token is shared with its designated voter. These Auth Tokens function like traditional API Authorization Tokens and must be kept confidential.

Election Administrators have the authority to invalidate individual Auth Tokens and generate new ones as needed, such as in cases where a voter accidentally leaks or loses their Auth Token.

After the election, all used Auth Tokens can be audited.

### b. Finalize Ballot Content

Similar to traditional paper elections, the Election Administrator must finalize the questions and options that appear on the ballot.

The SIV Admin software offers a user-friendly Point-and-Click Ballot Designer interface and a machine-readable JSON Schema interface for advanced editing.

Each ballot item should have a unique key, such as "president", "governor", "mayor", or "proposition_3".

SIV is fully compatible with alternative voting methods like Ranked Choice Voting and Approval Voting. Real-time user-interface feedback and invalid ballot prevention features make it easier to adopt these less common methods with SIV.
...
Verifying Observers do not need to trust each other and cannot tamper with votes.
...
For other voters that are easier to reach via physical mail, Election Administrators can elect to continue to send them physical letters with unique codes per voter to verify their receipt.
...
Regardless of the verification method used, the entire process leaves a written audit trail for independent verification.
...
### Step 2. Voters Make Selections & Vote Encryption

Once voters have been successfully authenticated, they can view their blank ballot.

They can make their selections with a simple point-and-click interface, similar to the ease of filling out online forms such as Google Form.

#### Encryption Overview

The Secure Interactive Voting (SIV) Voter software automatically encrypts each vote selection using Elliptic Curve ElGamal Encryption over the Ristretto255 prime-order subgroup of Curve25519 (IETF RFC 8031), offering a NIST-recommended security level of 128 bits and classified as a "SafeCurve."

The Public Key used, which SIV calls the "Encryption Address", is the Threshold Public Key generated by the Verifying Observers in the [Pre-C Ceremony](/technical-specifications#c-register-verifying-observers).
...
4. Converting this `length + encoded string + random bits` to a hexadecimal string, and then checking if it is a valid Ristretto point. If it is not, the embed function loops back again to Step 1 to generate new random bits to try again until it succeeds, which is approximately 1/16th of the time.
...
The anonymization process, although conceptually straightforward, employs robust cryptographic techniques. Essentially, the objective is to shuffle the list of encrypted votes.

To prevent traceability during the shuffle, each vote undergoes re-encryption in a manner that maintains the original value but obscures any association with the initial encrypted vote. However, this alone does not guarantee anonymity, as the shuffler would still possess knowledge of the vote permutations.

To address this concern, each Verifying Observer performs a sequential shuffle, ensuring that no single party can discern the complete shuffling process.

This procedure necessitates an operation that accepts a list of encrypted votes as input and generates a shuffled list of re-encrypted votes as output. Crucially, this operation must enable external observers to verify the integrity of the shuffle via a zero-knowledge proof, confirming that the output list is a valid re-encryption of the input list.

We employ an algorithm derived from Dr. Andrew Neff's 2004 paper ["Verifiable Mixing (Shuffling) of ElGamal Pairs"](https://people.csail.mit.edu/rivest/voting/papers/Neff-2004-04-21-ElGamalShuffles.pdf). The primary modification involves substituting modular exponentiation with Elliptic Curve Point Multiplication.

The proofs of completeness, soundness, and zero-knowledge remain valid in the context of Elliptic Curve Point Multiplication, while this adaptation offers the advantage of a higher encryption strength-to-key-size ratio.
