...
The Election Administrator is responsible for defining a list of eligible voters. Each voter is assigned a unique Authorization Token ("Voter Auth Token").

Auth Tokens, which can only be used once, are necessary for casting a vote. They are designed to be infeasible to guess and are only valid for a single election. The SIV Admin Software automatically generates these tokens with cryptographically secure randomness when an Admin adds a new voter.

These tokens are currently 10-character hexadecimal strings (e.g., 2378bf376d), resulting in $16^{10}$ (= $2^{40}$) or slightly over a trillion possible combinations. Brute force attempts to crack auth tokens are not a significant risk, as IP-address-based validation logging and rate-limiting are in place.

Election Administrators have knowledge of all assigned Voter Auth Tokens. When the election begins, each Voter Auth Token is shared with its designated voter. These tokens function like traditional API Authorization Tokens and must be kept confidential.

Election Administrators have the authority to invalidate individual Auth Tokens and generate new ones as needed, such as in cases where a voter accidentally leaks or loses their token.
