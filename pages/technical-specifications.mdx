...
The SIV Voter software automatically encrypts each vote selection using Elliptic Curve ElGamal Encryption over the Ristretto255 prime-order subgroup of Curve25519 ([IETF RFC 8031](https://www.rfc-editor.org/rfc/rfc8031)), offering a NIST-recommended security level of 128 bits and classified as a "SafeCurve".
...
However, this alone does not guarantee anonymity, as the shuffler would still possess knowledge of the vote permutations. To address this, multiple Verifying Observer each perform a sequential shuffle, ensuring that no single party can know the complete shuffling order.
...
This decryption is done in another decentralized ceremony, involving a sufficient threshold number of Verifying Observers, with each Verifying Observer submitting a partial decryption of each vote.

Note that this decryption is also provable — meaning parties cannot cheat in such a way as to make the vote decrypt to something else without people knowing. These ZK Proofs of a Valid Partial Decryption are verifiable by everyone, and automatically checked by all the other Verifying Observers & SIV Admin Server.
