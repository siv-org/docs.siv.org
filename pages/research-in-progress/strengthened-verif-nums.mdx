...
- Complements existing [2nd-Device Checks](/verifiability/detecting-malware), which can catch tampered votes during the voting window.
...
This new strengthened verification number ability complements the existing [2nd-Device Checks](/verifiability/detecting-malware), which can detect on-device tampering even while the voting window is still open, but depend on alternative devices free of coordinating malware. RLAs can be used to sample across both — eg. inviting some 2nd-Device Checkers to also Strengthen — for independent calibration of 2nd Device Checks.
