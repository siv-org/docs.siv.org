...
import { MultipleBountyClaimersTable } from '../../components/MultipleBountyClaimers/Table'
...
After you cast your vote, you receive an unforgeably-signed Vote Submission Receipt. This is stored in the browser LocalStorage on your voting device, and also emailed to your address on file as an additional backup. If, for any reason, your vote does not make it into the final tally, you can use this signed receipt to claim a large financial payoff.
...
E.g. for a hypothetical $10k pot, with a 70% reward, the first 4 would be:
...
If it's been x days (e.g. 45) since Transaction 2 (Merkle tree) was posted without any successful claims, the funds are returned back to the address who posted Transaction 1.

_Only if needed, to make sure Transaction 2 is posted without delay:_ If it's been y days (e.g. 45) since Transaction 1 was posted (funds put into escrow) without a matching Transaction 2 (Merkle tree), anyone can claim the reward, without needing a signed submission receipt.
