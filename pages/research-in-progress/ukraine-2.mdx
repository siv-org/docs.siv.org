...
Due to the ongoing war, martial law is in effect, and elections have been cancelled. The decision cites high risks of physical danger, significant costs, and logistical challenges, particularly given the large number of Ukrainians engaged in the front lines or displaced abroad.
...
The [SIV Protocol](https://docs.siv.org/technical-specifications) has been developed to run free and fair elections that can withstand extreme levels of attack, enabling active proof of correctness, using well-vetted computer science discoveries.

The protocol, already developed, tested, and ready to use [^1], meets the criteria of Authenticated Voters, Verifiable Results, and Private Voting. It also provides powerful remediation and architecture for authentication attacks, malware, denial of service, server penetration, spoofing, privacy violations, and vote selling.

[^1]: Some tailored features still need to be implemented, but the overall infrastructure has already been successfully used in government-grade elections. It is 80% ready for the specific needs of Ukraine, believing we can run a provably legitimate election in November 2024.

### Deadline

We have the opportunity to act now.

1. Before the new status quo of anti-democratic no-elections ossifies in place.
