...
14. **Invalidation and remediation of compromised votes**: SIV's design allows any discovered compromised votes [to be invalidated, and if appropriate, remediated](/verifiability/remediating-compromised-votes).
...
18. **Secret paper overrides**: Because SIV works alongside paper methods, voters under coercion pressure could cast a 2nd vote using paper-methods, which can treated as overriding, without alerting the coercer. Although it must be noted this "secret paper overrides" configuration does sacrifice some verifiability (partial tally totals, not malware nor privacy).
