...
Many people mistakenly frame internet voting as uniquely vulnerable to risks of vote selling, in contrast to paper voting methods, as if it's a simple binary: `paper = no`, `internet = yes`.

But this seems far too reductionist.

In fact, paper methods are already vulnerable in multiple ways. Likewise, there are number of other mitigating factors that protect all methods, such as voters' inherent resistance & severe legal penalties if caught.

We have attempted to quantify these risks on a scale of 1-10 rather than binary, and came up with this analysis:

| Coercion resistance (*without bounties*) | SIV | Mail | In Person |
|------------------------------------------|:----:|:----:|:---------:|
| How protected are voters against attempts to threaten or purchase their vote selections? | 5/10 | 6/10 | 8/10 |

<i className='text-center block opacity-80'>Click into specific scores on [docs.siv.org/compare](https://docs.siv.org/compare) to see the basis for each.</i>

Below, we will explore multiple arguments on the current status of vote selling in government elections.

Finally, we will propose a strong potential solution to strengthen all methods' defenses against vote selling to be closer to:
